 ...
 *           The const and reference are not required, but key cannot be modified in
 * function.
 *      - K and M must be regular (copyable, default constructible, and equality
 * comparable).
 */
...
class HashMap {
     ...
     * to figure out the type of this iterator (you would've never guessed what the
     * template parameters are here), which is why the aliases are crucial.
     *
     * Usage:
    ...
    /*
     * Alias for the const_iterator type. Recall that it's impossible for an external
     * client to figure out the type of this iterator (you would've never guessed what
     * the template parameters are here), which is why the aliases are crucial.
     *
     * Usage:
     *      const auto& cmap = map;
     ...
     * (returns 0 or 1) to check if key exists. In C++20, map.contains(key) will be
     * available. Since contains feels more natural to students who've used the Stanford
     ...
     * Complexity: O(1) amortized average case, O(N) worst case, N = number of elements
     *
     * Notes: recall that operator[], which you will implement, does not throw
     * exceptions, if a key is not found. Instead, it will create a K/M pair for that
     * key with a default mapped value. This function is also not const-correct, which
     * you will fix in milestone 2.
     */
    M& at(const K& key);
     ...
     * Complexity: O(N), N = number of elements
     *
     * Notes: clear removes all the elements in the HashMap and frees the memory
     * associated with those elements, but the HashMap should still be in a valid state
     ...
     *                         this element may have been just added, or may have
     * already existed. bool - true if the element was successfully added, false if the
     * element already existed.
     *
     * Usage:
     *      HashMap<int, std::string> map;
     *      auto [iter1, insert1] = map.insert({3, "Avery"}); // inserts {3, "Avery"},
     * iter1 points to that element, insert1 = true auto [iter2, insert2] =
     * map.insert({3, "Anna"});  // no-op, iter2 points to {3, "Avery"}, insert2 = false
     *
     ...
     *
     * Usage:
     *      map.erase(3);           // assuming K = int, erases element with key 3,
     * returns true
     *
     * Complexity: O(1) amortized average case, O(N) worst case, N = number of elements
     *
     ...
     *
     * Complexity: O(N) amortized average case, O(N^2) worst case, N = number of
     * elements
     *
     * Notes: our minimal HashMap implementation does not support automatic rehashing,
     * but std::unordered_map will automatically rehash, even if you rehash to a very
     * small number of buckets. For this reason, std::unordered_map.rehash(0) is allowed
     * and forces an unconditional rehash. We will not require this behavior. If you
     * want, you could implement this.
     *
     * Previously, this function was part of the assignment. However, it's a fairly
     * challenging linked list problem, and students had a difficult time finding an
     * elegant solution. Instead, we will ask short answer questions on this function
     * instead.
     */
     ...
     * Complexity: O(N), N = number of elements.
     *
     * Notes: debug will not compile if either K or V does not support operator<< for
     * std::ostream. this function will crash if your linked list logic is incorrect
     * (eg. forgot to reset the last node's next to nullptr). Check where the source of
     * the compiler error comes from before complaining to us that our starter code
     * doesn't work!
     *
     ...
     *
     * Requirements: InputIt must be iterators to a container whose elements are pair<K,
     * M>.
     *
     * Usage:
    ...
    HashMap(InputIt first, InputIt last, size_t bucket_count = kDefaultBuckets,
            const H& hash = H());
     ...
     *
     * Notes: you may want to do some research on initializer_lists. The most important
     * detail you need to know is that they are very limited, and have three functions:
     * init.begin(), init.end(), and init.size(). There are no other ways to access the
     * elements in an initializer_list. As a result, you probably want to leverage the
     * range constructor you wrote in the previous function!
     *
     * Also, you should check out the delegating constructor note in the .cpp file.
     */
    HashMap(std::initializer_list<value_type> init,
            size_t bucket_count = kDefaultBuckets, const H& hash = H());
    ...
    struct node {
        ...
        /*
         * Constructor with default values, so even if you forget to set next to nullptr
         * it'll be fine.
         *
         * Usage:
     ...
     *
     * Usage:
     *      node* ptr = _buckets_array[index];          // _buckets_array is array of
     * node* const auto& [key, mapped] = ptr->value;     // each node* contains a value
     * that is a pair
     */
