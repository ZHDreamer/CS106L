 ...
 * V3 (winter 2021) - added iterator tests, factored out edge case and const-correctness
 * tests
...
void D_at() {
    ...
    // at returns a reference, so assigning their return value on the left-hand value
    // should change the contents of the map.
    answer.at("A") = 42;
    answer.at("B") = 137;
    answer.at("A") = -42;
...
template<typename T>
auto ensure_at_return_is_const(const T& map)
    -> decltype((void)(map.at(0) = 3), void()) {
...
void E_at_const_correct() {
    ...
    for (const auto& kv_pair : vec) {
        answer.insert(kv_pair);
        map1.insert(kv_pair);
        VERIFY_TRUE(check_map_equal(map1, answer),
                    __LINE__);  // both maps have same elements
    }
    ...
    for (const auto& [key, mapped] : answer) {
        ...
        VERIFY_TRUE(c_ref_map1.at(key) == mapped,
                    __LINE__);  // confirm parameter is a const reference
...
__attribute__((unused)) void unused_rehash_correct_by_time() {
    /*
     * This was previously a test when rehash was part of the assignment. However, it
     * did not work well with the compiler optimizations, so we will not include this
     * test. If a future lecturer wants to add rehash back into the assignment, they can
    ...
    using V = int;
    float tolerance =
        0.5;            // makes me feel like an engineer, probably using the term wrong
    ...
    VERIFY_TRUE(ratio6_23 < (1 + tolerance) * 1500 / 500
                    && ratio6_23 > 1 / (1 + tolerance) * 1500 / 500,
                __LINE__);
    VERIFY_TRUE(ratio6_54 < (1 + tolerance) * 6000 / 1500
                    && ratio6_54 > 1 / (1 + tolerance) * 6000 / 1500,
                __LINE__);
    VERIFY_TRUE(ratio3_10 < (1 + tolerance) * 1600 / 500
                    && ratio3_10 > 1 / (1 + tolerance) * 1600 / 500,
                __LINE__);
    VERIFY_TRUE(ratio3_21 < (1 + tolerance) * 7500 / 1600
                    && ratio3_21 > 1 / (1 + tolerance) * 7500 / 1600,
                __LINE__);
    VERIFY_TRUE(ratio2_10 < (1 + tolerance) * 7500 / 3000
                    && ratio2_10 > 1 / (1 + tolerance) * 7500 / 3000,
                __LINE__);

    // fun fact: we had to add an -O0 flag because the compiler was optimizing our code
    // a little too well. Turns out that the runtime all of these is the same with
    // optimization (!)
}
...
void K_iter_foreach_filled_buckets() {
    ...
    HashMap<int, int, decltype(identity)> map10(
        6, identity);  // can your iterator traverse normal use case?
    for (int i = 0; i < 10; ++i) {
        map10.insert({i, i});
...
void N_iter_forward_operators() {
    ...
    VERIFY_TRUE(iter->first * iter->first == iter->second,
                __LINE__);                   // behavior of -> operator
    VERIFY_TRUE(iter == iter, __LINE__);     // behavior of == operator
    VERIFY_TRUE(!(iter != iter), __LINE__);  // behavior of != operator

    (*iter).second = -1;  // behavior of * operator as an l-value
    VERIFY_TRUE((*iter).second == -1,
                __LINE__);                      // behavior of * operator as an r-value
    ...
    HashMap<int, int>::iterator iter0 =
        iter;             // just to prove why type aliases are helpful
...
void P_iter_algorithm_permutation() {
    /* Stress test of the multipass functionality of forward iterators using STL
     * is_permutation */
    ...
    VERIFY_TRUE(
        std::is_permutation(map1.begin(), map1.end(), answer.begin(), answer.end()),
        __LINE__);
    VERIFY_TRUE(
        std::is_permutation(answer.begin(), answer.end(), map1.begin(), map1.end()),
        __LINE__);
    VERIFY_TRUE(
        std::is_permutation(map2.begin(), map2.end(), answer.begin(), answer.end()),
        __LINE__);
    VERIFY_TRUE(
        std::is_permutation(answer.begin(), answer.end(), map2.begin(), map2.end()),
        __LINE__);
    VERIFY_TRUE(std::is_permutation(map2.begin(), map2.end(), map1.begin(), map1.end()),
                __LINE__);
    VERIFY_TRUE(std::is_permutation(map1.begin(), map1.end(), map2.begin(), map2.end()),
                __LINE__);
    VERIFY_TRUE(std::is_permutation(map1.begin(), map1.end(), map1.begin(), map1.end()),
                __LINE__);
    VERIFY_TRUE(std::is_permutation(map2.begin(), map2.end(), map2.begin(), map2.end()),
                __LINE__);
    ...
    VERIFY_TRUE(
        ...
        __LINE__);
    VERIFY_TRUE(
        !std::is_permutation(map2.begin(), map2.end(), map1.begin(), map1.end()),
        __LINE__);
    VERIFY_TRUE(
        !std::is_permutation(map1.begin(), map1.end(), map2.begin(), map2.end()),
        __LINE__);
    VERIFY_TRUE(std::is_permutation(map1.begin(), map1.end(), map1.begin(), map1.end()),
                __LINE__);
    VERIFY_TRUE(std::is_permutation(map2.begin(), map2.end(), map2.begin(), map2.end()),
...
void Q_iter_const_iterator() {
    /* Tests the const-correct of your iterator class by asking for const_iterators via
     * begin() and end() */
    std::set<std::pair<int, int>> questions {
        {1, 1},
        {2, 2},
...
void R_iter_const_correct() {
    ...
    VERIFY_TRUE((*c_iter).second == -1,
                __LINE__);  // behavior of * operator as an r-value
    c_iter->second = -2;    // behavior of -> operator as an l-value
    VERIFY_TRUE(c_iter->second == -2,
                __LINE__);  // behavior of -> operator as an r-value

    // these should not compile:
    // *iter = {0, 0};  // *iter is a std::pair<const K, M>, since K is const, = is
    // deleted
    // ++c_iter;        // ++ is non-const
...
void S_iter_const_conversion() {
    ...
    HashMap<int, int>::iterator       iter = map.begin();
    HashMap<int, int>::const_iterator converted_iter =
        iter;  // this results in an implicit conversion to const_iterator
    VERIFY_TRUE(converted_iter == c_iter, __LINE__);
    ++c_iter;
    ++iter;
    ++converted_iter;

    VERIFY_TRUE(static_cast<HashMap<int, int>::const_iterator>(iter) == c_iter,
                __LINE__);
    VERIFY_TRUE(static_cast<HashMap<int, int>::const_iterator>(iter) == converted_iter,
                __LINE__);
...
void T_iter_find_member() {
    /*
     * Verifies functionality of the find member function, which uses the iterator
     * class.
     */
    HashMap<std::string, int> map;
...
void V_iter_erase_member() {
    /*
     * Verifies functionality of the erase and insert. Note that this erase test takes
     * in a parameter that is a const_iterator, different from test G which only takes
     * in a key.
     */
    HashMap<std::string, int> map;
...
void A_range_ctor_basic() {
    ...
    std::unordered_map<std::string, int> answer {
        values.begin()++, values.end()};  // skips the first "Ignore me" pair
    HashMap<std::string, int> map {values.begin()++, values.end()};

    VERIFY_TRUE(check_map_equal(map, answer),
                __LINE__);  // maps should be equal, if not, you didn't transfer all the
                            // elements!
}
#endif
...
auto ensure_no_const_index_op(const T& map) -> decltype((void)(map[0]), void()) {
    VERIFY_TRUE(false, __LINE__);
    // The idea of this test is that there are two overloads for
    // ensure_no_const_index_op. The compiler tries this current overload, which only
    // compiles if map[0] compiles, but it is not supposed to because map a const
    // reference and map[0] may autoinsert.

    // If map[0] does not compile (as expected), then
    // the varadic template overload above is called, and no runtime assertion happens.

    // It's tricky to test "this code should not compile" - this is a hacky way to check
    // that.
}
...
void C_stream_insert_operator() {
    ...
    map.insert({"Avery", 3});
    oss3 << map;
    VERIFY_TRUE(oss3.str() == "{Avery:3, Anna:2}" || oss3.str() == "{Anna:2, Avery:3}",
                __LINE__);
    auto s = oss3.str();
    ...
    oss5 << reverse;
    VERIFY_TRUE(oss5.str() == "{3:Avery, 2:Anna}" || oss5.str() == "{2:Anna, 3:Avery}",
                __LINE__);
    ...
    oss << "A" << map << 3 << reverse << '\n' << map << map << '\n';
    VERIFY_TRUE(
        oss.str() == "A{Avery:4}3{3:Avery, 2:Anna}\n{Avery:4}{Avery:4}\n"
...
void E_equality_operator() {
    ...
    VERIFY_TRUE(
        !(map1 != map2) && !(map2 != map1) && !(map1 != map1) && !(map2 != map2),
        __LINE__);
    ...
    for (int i = -1; i < 100; ++i) {
        map1.insert({i, i * i});
        map2.insert({i, i * i});
        VERIFY_TRUE(map1 == map2 && map2 == map1 && map1 == map1 && map2 == map2,
                    __LINE__);
        VERIFY_TRUE(
            !(map1 != map2) && !(map2 != map1) && !(map1 != map1) && !(map2 != map2),
            __LINE__);
...
void F_equality_const_correct() {
    ...
    VERIFY_TRUE(map1 == map2 && map2 == map1 && map1 == map1 && map2 == map2, __LINE__);
    VERIFY_TRUE(c_ref_map1 == map2 && map2 == c_ref_map1 && c_ref_map1 == c_ref_map1,
                __LINE__);
    VERIFY_TRUE(c_ref_map2 == map1 && map1 == c_ref_map2 && c_ref_map2 == c_ref_map2,
                __LINE__);
    ...
    VERIFY_TRUE(
        ...
        __LINE__);
    VERIFY_TRUE(
        !(c_ref_map1 != map2) && !(map2 != c_ref_map1) && !(c_ref_map1 != c_ref_map1),
        __LINE__);
    VERIFY_TRUE(
        !(c_ref_map2 != map1) && !(map1 != c_ref_map2) && !(c_ref_map2 != c_ref_map2),
        __LINE__);
    VERIFY_TRUE(!(c_ref_map2 != c_ref_map1) && !(c_ref_map1 != c_ref_map2), __LINE__);
...
void A_copy_ctor_basic() {
    ...
    for (const auto& [key, mapped] : vec) {
        ...
        VERIFY_TRUE(check_map_equal(copy_constructed, copy_constructed_answer),
                    __LINE__);
        VERIFY_TRUE(check_map_equal(map, answer), __LINE__);
    }
...
void C_copy_edge() {
    ...
    #pragma GCC diagnostic push
        ...
        map1.insert({"B", 2});
        map2.insert({"C", 3});
        std::unordered_map<std::string, int> answer1 {
            {"A", 1},
            {"B", 2}
        };
        std::unordered_map<std::string, int> answer2 {
            {"C", 3}
        };
        ...
        answer1 = answer2 = answer1 = (answer1 = answer1 = answer2) =
...
void D_move_ctor_basic() {
    ...
    for (const auto& [key, mapped] : vec) {
        ...
        VERIFY_TRUE(check_map_equal(move_constructed, move_constructed_answer),
                    __LINE__);
    }
...
void F_move_edge() {
    ...
    #pragma GCC diagnostic push
        ...
        (map1 = std::move(map1)) =
            std::move(std::move(map1) = (std::move(map1) = map1 = std::move(map1)));
        VERIFY_TRUE(check_map_equal(map1, answer1), __LINE__);
        VERIFY_TRUE(check_map_equal(map2, answer2), __LINE__);
    #pragma GCC diagnostic pop
        // your code could crash at this line due to memory problems!
...
int A_benchmark_insert_erase() {
    ...
    for (size_t size : sizes) {
        ...
        size_t my_map_result, std_map_result;
        {
            ...
            HashMap<int, int, decltype(good_hash_function)> my_map(size,
                                                                   good_hash_function);
...

        {
            ...
            std::unordered_map<int, int, decltype(good_hash_function)> std_map(
                size, good_hash_function);
            for (int element : million) {
        ...
        std::cout << " | HashMap: " << std::setw(13)
                  << print_with_commas(my_map_result);
        std::cout << " | std:unordered_map: " << std::setw(13)
                  << print_with_commas(std_map_result) << '\n';
